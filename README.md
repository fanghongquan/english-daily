...
每天通过飞书机器人推一篇 CET-4 阅读文章。点开链接进入网页：
段段对照中文翻译，重点词加粗并标注美式音标，支持段落/全文朗读、阅读进度、
理解答题、单词/短语加入墨墨，以及读完后的三级难度反馈。
...
- 双击单词或先点起始词再点结束词，把单词/短语加入墨墨
- 理解自测完成后显示得分、错题和解析
- 读完选择「有点简单 / 刚刚好 / 有点吃力」，用于小幅调整后续文章
...
4. `.github/workflows/daily.yml` 已设好定时，目标送达时间是**北京时间 08:40 左右**。
   注意 cron 槽位设在北京 06:13，因为 GitHub 的定时触发实测会推迟 2-3 小时，
   槽位是按这个反推的（不是准时触发），改动原因见那段注释。
...
每篇文章末尾有三级反馈。系统综合你的选择、首次答题得分、阅读完成情况和本篇单词/短语
操作数量，派生下一篇的新词数和句子复杂度目标（正文长度固定约 500 词，不随能力变化）。
目标是大约理解 85%–90%，同时每篇遇到 5–8 个值得学习的新词。

- 只复用现有 SCF、私有 COS、GitHub Actions 和每天原本的一次 DeepSeek 生成，不增加模型调用。
- 手机端自动同步不会弹出访问码；没有已保存访问码时先留在本机，主动点击反馈时才允许输入。
- 客户端**不上传具体单词或短语**，只上传不重复的操作数量。
- 学习档案不可用时自动使用**默认平衡档案**，不会阻断每天发布。
- GitHub Actions Secret `PROFILE_READ_TOKEN` 与 SCF 同名环境变量必须一致，且不得写进网页。

为保持零新增预算，不新建云函数、存储桶、付费 Runner 或资源包，也不接受购买/升级提示。
详细配置和回滚方式见 `SCF_DEPLOY.md`。
...
- `--source ai`（**默认，每日推送用这个**）：用大模型按四级难度现写一篇，配好翻译、
  重点词、音标，内容版权归你，零风险。配 `ANTHROPIC_API_KEY` 或任意 OpenAI 兼容
  key（DeepSeek、通义、Kimi 等都行）即可。正文固定 500-580 词，题材从内置的
  多领域题材库（`get_article.py` 顶部的 `TOPICS`）按日期轮换。
...
> 2026-09-18~21 曾短暂改用「抓近期新闻 + AI 改写」（`--source news`，含 `news_source.py`）。
> 因为改写出的文章专有名词太多、读起来费劲，2026-09-22 整套移除，回到内置题材库轮换。
